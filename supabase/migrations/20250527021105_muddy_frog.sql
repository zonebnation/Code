/*
  # Code Canvas Complete Database Schema
  
  This migration creates all the necessary tables, functions, triggers, and security policies
  for the Code Canvas application, including:
  ...
  2. Projects System
     ...
     - File hierarchy and content management
  
  3. Collaboration Features
     - Project collaborators with permission levels
     - Collaboration invites
     - Real-time editing sessions
     - Cursor positions and edit operations
...
-- Create indexes
CREATE INDEX collaborative_sessions_project_id_idx ON collaborative_sessions(project_id);
CREATE INDEX collaborative_sessions_file_id_idx ON collaborative_sessions(file_id);
CREATE INDEX collaborative_sessions_user_id_idx ON collaborative_sessions(user_id);
CREATE INDEX cursor_positions_session_id_idx ON cursor_positions(session_id);
CREATE INDEX edit_operations_project_id_idx ON edit_operations(project_id);
CREATE INDEX edit_operations_file_id_idx ON edit_operations(file_id);
CREATE INDEX edit_operations_user_id_idx ON edit_operations(user_id);
CREATE INDEX edit_operations_timestamp_idx ON edit_operations(timestamp);
