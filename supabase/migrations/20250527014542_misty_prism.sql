/*
  # Fix Project Collaborators Policies

  This migration addresses the infinite recursion issue in project_collaborators policies
  by replacing them with simpler, non-recursive policies.
*/
...
-- Create simplified policies that avoid circular references with unique names

-- Read access: Project owner or the user themselves can see collaborator entries
CREATE POLICY "ProjectCollaboratorsViewPolicy_v2" 
...
CREATE POLICY "ProjectCollaboratorsInsertPolicy_v2" 
  ON project_collaborators 
  FOR INSERT 
  TO authenticated 
  WITH CHECK (
...
CREATE POLICY "ProjectCollaboratorsUpdatePolicy_v2" 
  ON project_collaborators 
  FOR UPDATE 
  TO authenticated 
  USING (
...
CREATE POLICY "ProjectCollaboratorsDeletePolicy_v2" 
  ON project_collaborators 
  FOR DELETE 
  TO authenticated 
  USING (
