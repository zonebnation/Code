/*
  # Initial Schema Setup for Code Canvas Platform
  
  A comprehensive migration that creates the database schema for the entire application:
  1. User Management - profiles, follows, auth triggers
  2. Projects System - projects, files, file organization
  3. Collaboration - collaborators, invitations, realtime features
  4. Social Features - videos, likes, comments, tags
  5. Notifications - notification system with triggers
  6. Settings & Sharing - user settings and shared content
  7. Storage Policies - avatar, video and file storage
*/
...
DROP POLICY IF EXISTS "Users can read files from owned, collaborative or public projects" ON project_files;
...
DROP POLICY IF EXISTS "Users can view collaborative sessions for their projects or where they are collaborators" ON collaborative_sessions;
...
DROP POLICY IF EXISTS "Users can create collaborative sessions for projects they can access" ON collaborative_sessions;
...
DROP FUNCTION IF EXISTS create_notification(UUID, UUID, TEXT, TEXT, UUID, TEXT) CASCADE;
...
-- Create notifications trigger functions
...
DROP FUNCTION IF EXISTS notify_on_collaboration_invite() CASCADE;
...
DROP FUNCTION IF EXISTS handle_collaboration_invite_update() CASCADE;
...
BEGIN
  ...
  ELSE
    ...
    SET 
      ...
      allowed_mime_types = '{video/mp4,video/webm,video/ogg,video/quicktime,application/mp4}'
    WHERE name = 'videos';
  END IF;
