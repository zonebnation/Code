...
-- Policies for project files (create only if they don't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_files' AND policyname = 'Users can read files from owned, collaborative or public projects') THEN
    EXECUTE $POLICY$
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_files' AND policyname = 'Users can add files to owned or writable projects') THEN
    EXECUTE $POLICY$
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_files' AND policyname = 'Users can update files in owned or writable projects') THEN
    EXECUTE $POLICY$
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_files' AND policyname = 'Users can delete files from owned or writable projects') THEN
    EXECUTE $POLICY$
...
DO $$ 
BEGIN
  -- Only create if they don't exist to avoid dropping dependencies
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'notify_on_follow') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'notify_on_comment') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'notify_on_like') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'notify_on_collaboration_invite') THEN
    CREATE FUNCTION notify_on_collaboration_invite()
    RETURNS TRIGGER AS $$
    BEGIN
      PERFORM create_notification(
        NEW.invitee_id,
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'handle_collaboration_invite_update') THEN
    CREATE FUNCTION handle_collaboration_invite_update()
    RETURNS TRIGGER AS $$
    BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'notify_on_message') THEN
